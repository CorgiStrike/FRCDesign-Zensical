...
<script>
  function initGameWidget() {
    ...
    let inclusiveMode = inclusiveToggle.checked;
    ...
    function updateList() {
      ...
      const filtered = games.filter(g => {
        ...
        let matchesType, matchesTags;

        if (inclusiveMode) {
          matchesType =
            selectedTypes.size === 0 ||
            g.type.some(t => selectedTypes.has(t));

          matchesTags =
            selectedTags.size === 0 ||
            g.tags.some(t => selectedTags.has(t.name));

        } else {
          matchesType = [...selectedTypes].every(t => g.type.includes(t));
          matchesTags = [...selectedTags].every(tag => g.tags.some(t => t.name === tag));
        }
    ...
    resetButton.onclick = () => {
      resetSpin -= 360;
      resetButton.style.transition = "transform 0.2s linear";
      resetButton.style.transform = `rotate(${resetSpin}deg)`;
      ...
      inclusiveMode = false;
      descToggleCheckbox.checked = false;
...
<style>
  ...
  /* Inclusive and Description Toggle */
  #inclusive-toggle-label,
  #desc-toggle-label {
    display:inline-flex; 
    align-items:center; 
    gap:4px; 
  ...
  #inclusive-toggle-label input[type="checkbox"],
